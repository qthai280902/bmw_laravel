<x-admin-layout>
    <div class="mb-12">
        <h1 class="text-6xl font-light uppercase tracking-tighter mb-2">Edit <span class="font-black text-[#1C69D4]">Line</span></h1>
        <p class="text-zinc-500 font-medium font-outfit">Cập nhật thông tin dòng xe: <span class="text-white">{{ $category->name }}</span></p>
    </div>
            ...
            <x-admin.card>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $category->name) }}" required class="w-full bg-black border-zinc-800 text-white text-sm focus:border-[#1C69D4] focus:ring-0">
                    </div>

                    <div>
                        <label class="block text-[10px] font-black uppercase text-zinc-500 mb-2">Mô tả</label>
                        <textarea name="description" rows="4" class="w-full bg-black border-zinc-800 text-white text-sm focus:border-[#1C69D4] focus:ring-0">{{ old('description', $category->description) }}</textarea>
                    </div>
                </div>
            </x-admin.card>
